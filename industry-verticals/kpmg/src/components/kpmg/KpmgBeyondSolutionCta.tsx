'use client';

import type { JSX } from 'react';
import type { RichTextField, TextField } from '@sitecore-content-sdk/nextjs';
import { Link as SitecoreLink, Text } from '@sitecore-content-sdk/nextjs';
import { ComponentProps } from '@/lib/component-props';
import { useEditingHydrationProps } from './kpmg-editing-hydration';
import { useKpmgBeyondSolutionRouteFields } from './kpmg-beyond-solution-route-fields';

export type KpmgBeyondSolutionCtaProps = ComponentProps;

export const Default = (props: KpmgBeyondSolutionCtaProps): JSX.Element | null => {
  const id = props.params.RenderingIdentifier;
  const { styles } = props.params;
  const fields = useKpmgBeyondSolutionRouteFields();
  const editingHydration = useEditingHydrationProps();
  const ctaText = fields.CtaText?.value?.toString().trim();
  const ctaHref = fields.CtaLink?.value?.href?.trim();

  if (!ctaText && !ctaHref) {
    return null;
  }

  return (
    <section
      key={id ?? props.rendering?.uid}
      {...editingHydration}
      id={id}
      data-cy="solution-cta"
      className={[
        'component kpmg-beyond-solution-cta mx-auto w-full max-w-[860px] px-5 pb-12 xl:px-[60px]',
        styles || '',
      ].join(' ')}
    >
      <div className="flex flex-col gap-6 border border-white/20 bg-kpmg-bgCard p-6 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0 flex-1">
          <Text tag="h2" field={fields.CtaText} className="m-0 text-xl font-semibold text-white" />
        </div>
        <SitecoreLink
          field={fields.CtaLink}
          className="inline-flex shrink-0 items-center justify-center rounded-full bg-kpmg-purple px-8 py-3 text-sm font-semibold text-white no-underline hover:opacity-90"
        />
      </div>
    </section>
  );
};
