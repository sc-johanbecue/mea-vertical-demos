...
export async function POST(request: Request) {
  ...
  let body: UnapproveRequest;
  try {
    body = (await request.json()) as UnapproveRequest;
  } catch {
  ...
  const result = await updateSitecoreItemField(commentId, 'AdminApproved', '0', language);
  if (!result.ok) {
