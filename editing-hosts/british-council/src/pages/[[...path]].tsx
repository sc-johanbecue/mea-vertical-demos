...
export const getStaticProps: GetStaticProps = async (context) => {
  ...
  try {
    if (context.preview && isDesignLibraryPreviewData(context.previewData)) {
      ...
      // Layout Service can return `{}` when a rendering/datasource fails to serialize
      // (e.g. General Link with linktype=internal and no item id). SDK then throws on
      // `layoutData.sitecore.route` — treat that as notFound rather than a 500.
      page = await client.getPreview(context.previewData);
    } else {
      page = await client.getPage(path, { locale: context.locale });
    }
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    console.error('Sitecore page fetch failed', { path, preview: context.preview, message, error });
    page = null;
  }
