#!/usr/bin/env node
/**
 * Generates Sitecore serialization YAML for JM3Collection (jm3site) components.
 * Usage: node authoring/scripts/Generate-Jm3Components.mjs
 */
...
const TPL = {
  ...
  contextItemResolver: "6ccbccde-5e95-44e4-ae66-cfe7d3a0589e",
  navLevelFrom: "1bb88840-5fb3-4353-ad8d-81136f6ff75a",
  navLevelTo: "a59325bb-5a27-46f9-b811-9d499715f3be",
};

/** Page template that holds article fields for context-item renderings. */
const ARTICLE_PAGE_TEMPLATE_ID = "d857b7ec-8648-4d16-ab55-16bfaa0157c8";
...
const COMPONENTS = [
  {
    ...
    defaults: {
      ...
      PrivacyLink:
        '<link text="Privacy policy" linktype="external" url="https://matthey.com/privacy" />',
    },
  },
  {
    name: "Header",
  ...
  },
  {
    ...
    defaults: {
      Link: '<link text="Support" linktype="external" url="https://matthey.com/about-us/contact-us" />',
      IconClass: "jmheader__callicon",
    },
  },
  {
    name: "Navigation",
  ...
  },
  {
    ...
    defaults: {
      ...
      Link4:
        '<link text="Careers" linktype="external" url="https://matthey.com/careers" />',
    },
  },
  {
  ...
  },
  {
    ...
    fields: [],
    placeholders: [
      { key: "carousel-slides", allowed: ["FeatureCarouselCard"] },
    ],
    variants: ["Default", "Animated"],
    defaults: {},
  },
  ...
  },
  {
    ...
    variants: ["Default", "Animated", "ListBlock"],
    defaults: {
      Title:
        "Johnson Matthey is a world leader in platinum group metals (PGMs).",
  ...
  },
  {
    ...
    variants: ["Default", "Inversed", "Animated"],
    defaults: {
      Title: "Developing the future of platinum group metals",
      Body: "We're partnering with leading platinum group metal (PGM) companies in a transformative programme, aiming to accelerate the development of new applications and harness the power of these metals.",
      Cta: '<link text="Read more" linktype="external" url="https://matthey.com/future-pgms-partnership" />',
      PlayVideoLabel: "Play Video",
      VideoLink:
        '<link text="Play Video" linktype="external" url="https://www.youtube.com/watch?v=vnjUaXJzgAc" />',
    },
  },
  {
  ...
  {
    name: "BackgroundPanelSection",
    dataFolder: "BackgroundPanelSections",
    group: "Page Content",
    fields: [
      { name: "Title", type: "Single-Line Text" },
      { name: "BackgroundImage", type: "Image" },
    ...
    variants: ["Default", "Animated"],
    defaults: {
      Title: "Core technical capabilities underpinning our business",
  ...
  },
  {
    ...
    variants: ["Default", "Inversed", "Animated", "Carousel"],
  ...
  },
  {
    ...
    variants: ["Default", "Animated"],
    defaults: { Title: "Experience", Value: "200+", Label: "year history" },
  },
  {
    name: "Breadcrumb",
    ...
    group: "Page Structure",
    fields: [
      { name: "ParentLabel", type: "Single-Line Text" },
      { name: "ParentLink", type: "General Link" },
      { name: "CurrentTitle", type: "Single-Line Text" },
    ],
    variants: ["Default"],
    defaults: {
      ParentLabel: "Media",
      ParentLink: '<link text="Media" linktype="external" url="/News" />',
  ...
  },
  {
    ...
    fields: [
      { name: "Title", type: "Single-Line Text" },
      { name: "FilterLabel", type: "Single-Line Text" },
      { name: "TotalEntries", type: "Single-Line Text" },
      { name: "CurrentPage", type: "Single-Line Text" },
      { name: "EntriesPerPage", type: "Single-Line Text" },
    ],
    placeholders: [{ key: "news-articles", allowed: ["NewsArticleCard"] }],
    variants: ["Default"],
    defaults: {
      Title: "News",
      FilterLabel: "Date",
  ...
  {
    name: "ArticleBodySection",
    contextItem: true,
    group: "Page Content",
    fields: [
      { name: "Body", type: "Rich Text" },
      { name: "LinkedInShareUrl", type: "General Link" },
      { name: "TwitterShareUrl", type: "General Link" },
    ],
    variants: ["Default"],
    defaults: {
      Body: "<p>Article body copy.</p>",
    },
  },
];

function stableGuid(seed) {
  // Prefix MUST stay `jm3-` — shared with jm2 causes repo-wide duplicate IDs on push.
  ...
  function walk(dir) {
    for (const name of readdirSync(dir)) {
      ...
      if (statSync(full).isDirectory()) walk(full);
        ...
        templateFiles.push(readFileSync(full, "utf8"));
    }
  }
  walk(join(ROOT, "templates"));

  const missing = results
    .filter((r) => !r.contextItem)
    .filter(
      (r) =>
        !templateFiles.some((body) =>
          body.includes(`ID: "${r.ids.templateId}"`),
        ),
    )
...
function idsForComponent(comp) {
  ...
  const placeholderIds = Object.fromEntries(
    (comp.placeholders ?? []).map((p) => [
      p.key,
      stableGuid(`${n}-ph-${p.key}`),
    ]),
  );
  return {
    branchId,
...
`,
  ...
  const dsQuery = comp.dataFolder
    ? `query:$site/*[@@name='Data']/*[@@name='${comp.dataFolder}']|query:$sharedSites/*[@@name='Data']/*[@@name='${comp.dataFolder}']`
    : "";
...
- ID: "b0b15510-b138-470e-8f33-8da2e228aafe"
  Hint: Rendering Contents Resolver
  Value: "${guidUpper(TPL.contextItemResolver)}"
`
      : "";
...
`,
  ...
  for (const ph of comp.placeholders ?? []) {
    ...
    const allowed = ph.allowed
      .map((name) => guidUpper(stableGuid(`${name}-rendering`)))
      .join("\n    ");
    files.push(
      await writeYaml(
...
`,
  ...
  return {
    ...
    group: comp.group,
    files,
    contextItem: !!comp.contextItem,
  };
}

async function main() {
  const filter = process.argv.slice(2);
  const components = filter.length
    ? COMPONENTS.filter((c) => filter.includes(c.name))
    : COMPONENTS;
  if (filter.length && !components.length) {
    throw new Error(`No components matched filter: ${filter.join(", ")}`);
  }
  ...
  for (const comp of components.filter((c) => !c.contextItem && c.dataFolder)) {
  ...
  for (const [groupName, relPath] of [
    [
      "Page Structure",
      `${SITE_REL}/Presentation/Available Renderings/Page Structure.yml`,
    ],
    [
      "Page Content",
      `${SITE_REL}/Presentation/Available Renderings/Page Content.yml`,
    ],
    [
      "Navigation",
      `${SITE_REL}/Presentation/Available Renderings/Navigation.yml`,
    ],
  ]) {
    const filePath = join(ROOT, relPath);
    let content = await readFile(filePath, "utf8");
    const newGuids = results
      .filter((r) => r.group === groupName)
      .map((r) => guidUpper(r.ids.renderingId));
    ...
    console.log(
      `Updated ${groupName} available renderings (+${newGuids.length})`,
    );
  }

  const idsPath = join(__dirname, "jm3-component-ids.json");
  let map = {};
  try {
    map = JSON.parse(await readFile(idsPath, "utf8"));
  } catch {
    /* first run */
  }
  for (const r of results) {
    map[r.comp] = r.contextItem
      ? {
          renderingId: r.ids.renderingId,
          contextItem: true,
          pageTemplateId: ARTICLE_PAGE_TEMPLATE_ID,
